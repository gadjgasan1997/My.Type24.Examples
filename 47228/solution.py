...
# Идем по строке до предпоследнего символа (чтобы s[i+1] не выдал ошибку)
# Цикл for не поляет менять значение i руками внутри самого цикла
while i < len(full_text) - 1:

    # Если текущий символ — согласная, а следующий — гласная
    if is_consonant(full_text[i]) and is_vowel(full_text[i + 1]):
        current_streak += 1

        # Обновляем рекорд
        max_streak = max(max_streak, current_streak)

        # Прыгаем через 2 символа (всю цепочку целиком)
        i += 2
    else:
